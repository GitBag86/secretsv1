    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-6 max-w-5xl mx-auto">
        ...
        {!isLoading && (
          <div>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
              <StatCard href="/notes" label="Notes" value={stats.totalNotes} sub={`${stats.pinnedNotes} pinned · ${stats.notebookCount} notebooks`} icon="📝" color="from-blue-500/20 to-blue-600/10" />
              <StatCard href="/todos" label="Todos" value={stats.totalTodos} sub={`${stats.activeTodos} active · ${stats.completedTodos} done`} icon="✅" color="from-green-500/20 to-green-600/10" />
              <StatCard href="/todos" label="Priority" value={stats.highPriority} sub={`${stats.overdueTodos} overdue · ${stats.dueSoon} due soon`} icon="🔥" color="from-orange-500/20 to-orange-600/10" />
              <StatCard href="/calendar" label="Events" value={stats.totalEvents} sub={`${stats.upcomingEvents.length} upcoming this week`} icon="📅" color="from-purple-500/20 to-purple-600/10" />
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 mb-8">
              <div className="rounded-xl border bg-card p-6 shadow-sm">
                ...
                <div className="h-3 w-full bg-muted rounded-full overflow-hidden mb-3">
                  <div className="h-full bg-primary rounded-full transition-all duration-500" style={{ width: `${stats.todoCompletion}%` }} />
                </div>
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>{stats.todoCompletion}% complete</span>
                  <span>
                    {stats.overdueTodos > 0 && <span className="text-destructive font-medium">{stats.overdueTodos} overdue · </span>}
                    {stats.dueSoon - stats.overdueTodos > 0 && <span className="text-orange-500">{stats.dueSoon - stats.overdueTodos} due in 3 days</span>}
                ...
                {stats.recentNotes.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {stats.recentNotes.map((n) => (
                      <Link key={n.id} href="/notes" className="block p-2 -mx-2 rounded-md hover:bg-accent/50 transition-colors">
                        ...
                        return (
                          <div key={label} className="flex-1 flex flex-col items-center gap-1">
                            <div className="relative w-full flex items-end justify-center" style={{ height: '112px' }}>
                              <div className={`w-full max-w-[40px] ${color} rounded-t-md transition-all duration-500`} style={{ height: `${height}%` }} />
                            </div>
                ...
                {tags.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {stats.popularTags.map(({ tagId, count }) => {
                      ...
                      if (!tag) return null;
                      return (
                        <div key={tagId} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium" style={{ backgroundColor: tag.color + "18", color: tag.color }}>
                          <span>{tag.name}</span>
                          <span className="ml-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold" style={{ backgroundColor: tag.color + "30", color: tag.color }}>
                            {count}
                          </span>
                        </div>
                      );
